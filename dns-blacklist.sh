...
wget -O /tmp/bad-list.txt 'http://www.networksec.org/grabbho/block.txt'

# compile into single unique list
echo Cleaning up domains
#sed -e '/^$/d' -e '/^\#/d' -i /tmp/ads-list.txt
sed -e '/^$/d' -e '/^\#/d' -e 's/[^\s]*\s//' -i ads-list.txt
sed -e '/^$/d' -e '/^\#/d' -i /tmp/mal-list.txt
sed -e '/^$/d' -e '/^\#/d' -i /tmp/ran-list.txt
sed -e '/^$/d' -e '/^\#/d' -i /tmp/bad-list.txt
cat /tmp/ads-list.txt /tmp/bad-list.txt /tmp/mal-list.txt /tmp/ran-list.txt | sort | uniq > /tmp/blacklist.txt
